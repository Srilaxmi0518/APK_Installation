# Generated on 2026-02-12T15:54:25.583751 UTC
...
tap_send_button() {
    ...
    [ -z "$SEND_BOUNDS" ] && echo "[ERROR] Send button not found" && exit 1
...
get_latest_mms_info() {
    mms_info=$(su -c 'content query --uri content://mms --projection _id,msg_box,resp_st,date --sort "date DESC" | head -n 1' \
        | sed -n 's/Row: *\([0-9][0-9]*\).*msg_box=\([0-9][0-9]*\).*resp_st=\([0-9A-Z]*\).*date=\([0-9][0-9]*\).*/\1 \2 \3 \4/p')
    echo "$mms_info"
}
...
[ -z "$IMG_ID" ] && echo "[ERROR] Image ID not found" && exit 1
