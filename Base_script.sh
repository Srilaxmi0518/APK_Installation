...
monitor_call_state() {
    ...
    while true; do
        ...
        if [ "$elapsed" -ge "$DURATION" ]; then
            echo "[INFO] Call monitored for $DURATION seconds"
            break
        fi
        mCallState=$(su -c "dumpsys telephony.registry \
            | grep 'mCallState' \
            | sed 's/.*=\(.*\)/\1/' \
            | sed -n '1p'")
        case "$mCallState" in
            ...
            1) echo "[CALL] RINGING" ;;
            2) echo "[CALL] OFFHOOK" ;;
            *) echo "[CALL] UNKNOWN ($mCallState)" ;;
        esac
        if [ "$mCallState" -ne 2 ]; then
            echo "[ERROR] Call state changed ($mCallState). Breaking loop."
            break
        fi
        echo "[INFO] CALL continued ($elapsed sec)"
        sleep "$INTERVAL"
    done
}
...
voice_service(){
  ...
  sleep 5
  monitor_call_state "$call_duration"
#  sleep call_duration
...
main_loop() {
    while true; do
        network_check
        [ "$ENABLE_DATA" = true ]  && data_service
        [ "$ENABLE_VOICE" = true ] && voice_service "$VOICE_DURATION"
        [ "$ENABLE_SMS" = true ]   && sms_service
        [ "$ENABLE_MMS" = true ]   && mms_service
        self_update
        sleep 2
    done
}
